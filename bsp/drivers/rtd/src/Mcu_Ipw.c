...
#define MCU_IPW_AR_RELEASE_MINOR_VERSION_C       9
#define MCU_IPW_AR_RELEASE_REVISION_VERSION_C    0
#define MCU_IPW_SW_MAJOR_VERSION_C               7
...
Std_ReturnType Mcu_Ipw_InitRamSection(const Mcu_RamConfigType * RamConfigPtr)
{
    ...
    if (RAM_IP_STATUS_OK == Ram_Ip_InitRamSection(RamConfigPtr))
    {
        RamStatus = E_OK;
    }
    else /* RAM_IP_STATUS_NOT_OK */
    {
        RamStatus = E_NOT_OK;
    }
...
Mcu_PllStatusType Mcu_Ipw_GetPllStatus(void)
{
    ...
    return (Mcu_PllStatusType)Clock_Ip_GetPllStatus();
}
#endif /* (MCU_NO_PLL == STD_OFF) */
...
Mcu_RamStateType Mcu_Ipw_GetRamState(void)
{
    ...
    if (RAM_IP_RAMSTATE_VALID == Ram_Ip_GetRamState())
    {
        RamState = MCU_RAMSTATE_VALID;
    }
    else /* RAM_IP_RAMSTATE_INVALID */
    {
