...
#define MCU_AR_RELEASE_MINOR_VERSION_C       9
#define MCU_AR_RELEASE_REVISION_VERSION_C    0
#define MCU_SW_MAJOR_VERSION_C               7
...
#if defined(MCU_MULTIPARTITION_SUPPORT)
#if (MCU_MULTIPARTITION_SUPPORT == STD_ON)
    if ( u8UserId != ((*Mcu_pConfigPtr->ClockConfigArrayPtr)[ClockConfigId]).ClkPartitionId )
    {
        CheckStatus = (Std_ReturnType)E_NOT_OK;
...
static Std_ReturnType Mcu_CheckPerformReset(void)
{
    ...
    if ( NULL_PTR == Mcu_pConfigPtr->HwIPsConfigPtr )
    {
        CheckStatus = (Std_ReturnType)E_NOT_OK;

        (void) Det_ReportError((uint16)MCU_MODULE_ID, MCU_INSTANCE_ID, MCU_PERFORMRESET_ID, MCU_E_PARAM_POINTER);
    }
...
static Std_ReturnType Mcu_CheckConfigureGpren(uint8 Module)
{
    ...
    if ( (uint32)Module >= eMIOS_INSTANCE_COUNT )
    {
        CheckStatus = (Std_ReturnType)E_NOT_OK;

        (void) Det_ReportError((uint16)MCU_MODULE_ID, MCU_INSTANCE_ID, MCU_EMIOSCONFIGUREGPREN_ID, MCU_E_EMIOS_INDEX_OUT_OF_RANGE);
    }
...
static Std_ReturnType Mcu_CheckDisableCMU(Clock_Ip_NameType ClockName)
{
    ...
    if (((uint16)ClockName) >= ((uint16)RESERVED_CLK))
    {
        CheckStatus = (Std_ReturnType)E_NOT_OK;
