...
#define POWER_IP_MC_RGM_AR_RELEASE_MINOR_VERSION_C       9
#define POWER_IP_MC_RGM_AR_RELEASE_REVISION_VERSION_C    0
#define POWER_IP_MC_RGM_SW_MAJOR_VERSION_C               7
#define POWER_IP_MC_RGM_SW_MINOR_VERSION_C               0
#define POWER_IP_MC_RGM_SW_PATCH_VERSION_C               1
...
#if (!(defined(POWER_IP_DERIVATIVE_008) || defined(POWER_IP_DERIVATIVE_009) || defined(POWER_IP_DERIVATIVE_010)))
...
Power_Ip_ResetType Power_Ip_MC_RGM_GetResetReason(void)
{
    ...
    /* If functional reset escalation to destructive reset is disabled, then the status of FES register must be ignored if
    the fields of Destructive Event Status Register (DES) other than DES[F_POR] are set. */
...
Power_Ip_RawResetType Power_Ip_MC_RGM_GetResetRawValue(void)
{
    ...
    the destructive reset was due to functional reset escalation or if it was triggered directly by a destructive reset
    source, in which case FES needs to be ignored */
    if (((uint32)0U == (Power_Ip_pxMC_RGM->DES & MC_RGM_DES_RWBITS_MASK32)) || (((uint32)0U != (Power_Ip_pxMC_RGM->DES & MC_RGM_DES_RWBITS_MASK32)) && ((uint32)0U != Power_Ip_pxMC_RGM->FRET)))
    {
        RegValue = Power_Ip_pxMC_RGM->FES & MC_RGM_FES_RWBITS_MASK32;
    }
    ...
    {
        PreviousMode = POWER_IP_SOC_STANDBY_MODE;
    }
    else
    {
  #endif
...
void Power_Ip_MC_RGM_ResetAltInt(void)
{
    ...
    if (POWER_MC_RGM_UNINIT != Power_Ip_eMcRgmStatus)
    {
        ...
        if ((uint32)0U != MaskedFesIntResetReason)
        {
            ...
            Power_Ip_pxMC_RGM->FES = ((uint32)(MaskedFesIntResetReason & MC_RGM_FES_RWBITS_MASK32));
