...
#define POWER_IP_PMC_AR_RELEASE_MINOR_VERSION_C       9
#define POWER_IP_PMC_AR_RELEASE_REVISION_VERSION_C    0
#define POWER_IP_PMC_SW_MAJOR_VERSION_C               7
...
void Power_Ip_PMC_PowerInit(const Power_Ip_PMC_ConfigType * ConfigPtr)
{
      ...
      defined(POWER_IP_DERIVATIVE_009) || defined(POWER_IP_DERIVATIVE_008) || defined(POWER_IP_DERIVATIVE_010))
    uint32 StartTime;
    uint32 ElapsedTime;
    uint32 TimeoutTicks;
    ...
    /* Clear LVD flag before config PMC_CONFIG */
    if ((uint32)0x00U != ((uint32)(ConfigPtr->ConfigRegister) & PMC_CONFIG_LVDIE_MASK))
    {
        IP_PMC->LVSC |= (uint32)PMC_LVSC_UV_IRQ_FLAGS_MASK32;
    }

    /* Clear HVD flag before config PMC_CONFIG */
    if ((uint32)0x00U != ((uint32)(ConfigPtr->ConfigRegister) & PMC_CONFIG_HVDIE_MASK))
    {
  ...
  #endif
#endif
#if (defined(POWER_IP_DERIVATIVE_006) || defined(POWER_IP_DERIVATIVE_009) || defined(POWER_IP_DERIVATIVE_008) || defined(POWER_IP_DERIVATIVE_010))
...
void Power_Ip_PMC_VoltageErrorIsr(void)
{
    ...
    uint32 VoltageIsrStatusFlags;

    /* Read Voltage Detect Flag */
    VoltageIsrStatusFlags = (uint32)((IP_PMC->LVSC) & PMC_LVSC_OV_UV_IRQ_FLAGS_MASK32);
    /* Read Low Voltage Detect Reset Enable */
    VoltageIsrEnabled = (uint32)IP_PMC->CONFIG;

    if (PMC_UNINIT != Power_Ip_ePMCStatus)
    {
        /* Check if there is any event to report */
        if (((VoltageIsrStatusFlags & PMC_LVSC_UV_IRQ_FLAGS_MASK32) != (uint32)0U) && \
            ((VoltageIsrStatusFlags & PMC_LVSC_OV_IRQ_FLAGS_MASK32) != (uint32)0U) && \
            ((VoltageIsrEnabled & PMC_CONFIG_LVDIE_MASK) == (uint32)PMC_CONFIG_LVD_INTERRUPTS_ENABLE) && \
            ((VoltageIsrEnabled & PMC_CONFIG_HVDIE_MASK) == (uint32)PMC_CONFIG_HVD_INTERRUPTS_ENABLE) \
            )
        {
            IP_PMC->LVSC |= (uint32)(PMC_LVSC_OV_UV_IRQ_FLAGS_MASK32);
        ...
        else if (((VoltageIsrStatusFlags & PMC_LVSC_UV_IRQ_FLAGS_MASK32) != (uint32)0U) && \
                 ((VoltageIsrEnabled & PMC_CONFIG_LVDIE_MASK) == (uint32)PMC_CONFIG_LVD_INTERRUPTS_ENABLE) \
                )
        {
            IP_PMC->LVSC |= (uint32)(PMC_LVSC_UV_IRQ_FLAGS_MASK32);
        ...
        else if (((VoltageIsrStatusFlags & PMC_LVSC_OV_IRQ_FLAGS_MASK32) != (uint32)0U) && \
                 ((VoltageIsrEnabled & PMC_CONFIG_HVDIE_MASK) ==(uint32)PMC_CONFIG_HVD_INTERRUPTS_ENABLE) \
                )
        {
            IP_PMC->LVSC |= (uint32)(PMC_LVSC_OV_IRQ_FLAGS_MASK32);
    ...
    {
        /* Clear the ISR flag if the Enable flag and Status flag are enabled */
        if (((VoltageIsrStatusFlags & PMC_LVSC_UV_IRQ_FLAGS_MASK32) != (uint32)0U) && \
            ((VoltageIsrStatusFlags & PMC_LVSC_OV_IRQ_FLAGS_MASK32) != (uint32)0U) && \
            ((VoltageIsrEnabled & PMC_CONFIG_LVDIE_MASK) == (uint32)PMC_CONFIG_LVD_INTERRUPTS_ENABLE) && \
            ((VoltageIsrEnabled & PMC_CONFIG_HVDIE_MASK) == (uint32)PMC_CONFIG_HVD_INTERRUPTS_ENABLE) \
...
/**
* @brief            This function to disable internal trickle LDO (V15 not kept on target in LPM)
* @details          This function is only used in Standby mode.
*
* @return           void
*
*/
void Power_Ip_PMC_DisableLDO(void)
{
    /* Disable the V15 in LPM */
    if (0U != (IP_PMC->SMPSCONFIG & PMC_SMPSCONFIG_LPM15EN_MASK))
    {
        IP_PMC->SMPSCONFIG &= ~(PMC_SMPSCONFIG_LPM15EN_MASK);
    }
}
  #endif
#endif
